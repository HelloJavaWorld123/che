...
public class OpenShiftProjectCleaner {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(OpenShiftProjectCleaner.class);
    ...
    if (pvcEnabled && !isNullOrEmpty(projectName))
      ...
      public void onEvent(WorkspaceRemovedEvent event) {
        ...
        try {
          ...
          LOG.error("Failed to cleanup workspace '{}' data cause: {}", workspaceId, ex.getMessage());
        }
      }
    });
  }
